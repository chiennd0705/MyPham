...
namespace BuyGroup365.Models.Member
{
    public class LoadDropdown
    {
        ...
        public string LoadNewGroup(long cat, long? newgroup)
        {
            NewsGroupBusiness _newGroupBussiness = new NewsGroupBusiness();
            var html = "";

            var listGroup = _newGroupBussiness.GetListNewsGroup();
            html += "<option  value=\"NULL\">-- Chọn một nhóm tin --</option>";
            foreach (var item in listGroup)
            {
                if (newgroup != null && newgroup == item.Id)
                {
                    html += "<option  selected=\"selected\" value=\"" +
                            item.Id + "\">" + item.NewsGroupName + "</option>";
                }
                else
                {
                    html += "<option  value=\"" + item.Id + "\">" + item.NewsGroupName + "</option>";
                }
            }
            return html;
        }

        public string LoadListSlider(long cat, long? newgroup)
        {
            AlbumsBusiness _albumGroupBussiness = new AlbumsBusiness();
            var html = "";

            var listGroup = _albumGroupBussiness.GetListSlider();
            html += "<option  value=\"NULL\">-- Chọn một slider --</option>";
            foreach (var item in listGroup)
            {
                if (newgroup != null && newgroup == item.Id)
                {
