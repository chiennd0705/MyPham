 ...
 * @@Modify Date: 2015-09-19 22:43:38
 * @@Function:
 */
"use strict";
(function () {
    jQuery.fn.magiccart = function (options) {
        ...
        *******************************/
            ...
            updateSidebarCart: function () {
                if ($$('.block-cart')[0]) {
                    var url = this.getMagicUrl('ajax/cart');
            ...
            updateShoppingCart: function () { // add cart in page checkout
                if ($$('body.checkout-cart-index div.cart')[0]) {
                    var url = this.getMagicUrl('ajax/checkout');
            ...
            updateMinicart: function (ajax) {
                var url = this.getMagicUrl('ajax/reloadCart');
            ...
            searchIdIdProduct: function (elObj) {
                ...
                if (magiccartEvent != undefined) {
                    var idProduct = magiccartEvent.replace('http://buygroup365.com/Ajax/QuickView/', '');

                    if (idProduct) idProduct = idProduct[0].replace(/[^\d]/gi, '');
                    else { // for page compare
                        idProduct = magiccartEvent.match(/QuickView(.?)+uenc/);
                        if (idProduct) idProduct = idProduct[0].replace(/[^\d]/gi, '');
                    }
